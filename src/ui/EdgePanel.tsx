...
export function EdgePanel() {
  ...
  return <AnimatePresence>{edge && <motion.aside initial={{ x: 32, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: 32, opacity: 0 }} style={{ position: 'fixed', right: 16, top: 96, bottom: 130, width: 340, maxWidth: 'calc(100vw - 32px)', zIndex: 6, padding: '16px 18px', border: `1px solid ${color.panelBorder}`, borderRadius: 12, background: color.panelBg, backdropFilter: 'blur(10px)', color: color.textSecondary, fontFamily: font.uiSans }}>
    <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><div><div style={{ color: color.goldActive, fontFamily: font.uiSerif, fontSize: 16 }}>{byId.get(edge.source) ?? edge.source}</div><div style={{ color: color.textMuted, margin: '3px 0' }}>↓ {edge.relation} ↓</div><div style={{ color: color.textPrimary, fontFamily: font.uiSerif, fontSize: 16 }}>{byId.get(edge.target) ?? edge.target}</div></div><button onClick={close} aria-label="关闭" style={{ color: color.textMuted, fontSize: 18 }}>×</button></header>
    <div style={{ marginTop: 18, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>{[['关系类型', edge.relation], ['主数值', edge.weight ?? 1], ['起点数值', edge.sourceValue ?? '—'], ['终点数值', edge.targetValue ?? '—']].map(([k, v]) => <div key={String(k)} style={{ padding: 9, borderRadius: 6, background: 'rgba(255,255,255,.05)' }}><div style={{ color: color.textMuted, fontSize: 10 }}>{k}</div><div style={{ marginTop: 3, color: color.goldActive }}>{String(v)}</div></div>)}</div>
